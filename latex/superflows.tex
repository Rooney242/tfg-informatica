...
% TODO (Guillermo): Revisa lo de grupo/departamento.
\group[spa]{Naudit}
\group[eng]{Naudit}
...
%Glosario
%IP, TTL, router, MAC, pcap, NAT, CI, TCP, UDP, buffer, JSON
...
%%Revisar si falta informacion en esta parte sobre si alguno de los programas mencionados tiene funcionalidad para reconstruccion de caminos. Puede ser interesante tambien citar documentacion o similar. Puede ser necesario añadir alguna referencia más
...
% TODO (Guillermo) Wireshark está también para Windows y macOS. Yo quitaría lo de "dentro del SO linux" para evitar que te hagan una pregunta para tocar las narices con esto.
...
\textit{Netflow} (citar) es un protocolo de red que fue desarrollado por Cisco Systems y que recolecta información sobre tráfico IP. Las herramientas que hacen uso de \textit{Netflow} son las que más se podrían acercar a los objetivos que se persiguen en este TFG. Sin embargo, existen una serie de cuestiones por lo que no se considera válido. % TODO (Guillermo) Apropiado mejor que válido, que puede dar la impresión que dices que netflow no vale para nada, cuando lo que quieres decir es que no vale para este TFG.

% TODO (Guillermo) lo de 'se trata de un protocolo' creo que no hace falta, porque aaparte Netflow no es sólo un protocolo.
...
\subsection{\textit{Routers} NAT} % TODO (Guillermo) Mejor 'Routers y Routers NAT', que se vea que son dos cosas distintas aunque relacionadas.
...
%%Explicar mejor como se detectará si se esta realizando un nateo (igual no usamos decremento de TTL para detectar el NAT en si)
...
% TODO (Guillermo): Intenta que las figuras queden más cerca del texto que las describen. Útil: https://www.overleaf.com/learn/latex/Positioning_of_Figures
...
\subsection{Problemas con el orden y la completitud de los paquetes} % TODO (Guillermo): Completitud de los flujos, no?
...
%%Preguntar por claridad y correccion del pseudocodigo. Hay que revisarlo, creo que queda muy complicado asi explicado.
%%Los forall de unresolved_devices son iteraciones en el diccionario de dos claves, no se muy bien como expresarlo en pseudocodigo
% Guillermo: 
%  - El forall de unresolved_devices yo lo veo bien. En el fondo iterator sobre el diccionario de dos claves es como iterar por una lista de pares (in_mac, out_mac).
%  - Puedes poner comentarios dentro del pseudocódigo si ves que es útil.
%  - En lugar de device[in_mac] pon device.in_mac, por ser consistente con lo de new_device.in_macs. 
%  - Por lo demás yo lo veo bien y se entiende sin problemas. También puede ser que yo estoy más familiarizado con el tema este, pero vamos, que no me preocuparía demasiado.
\begin{algorithm}[btp]
\begin{algorithmic}
...
%todo: quiza plantear esta subseccion como una lista no es correcto ya que queda muy larga y se podria estructurar en parrafos.
\begin{itemize}
...
Para estos dos casos se ha recurrido a la observación en casos experimentales. Hay que resaltar que estos parámetros son variables y que se pueden ajustar en función de las características específicas de cada red. En la \fref{sec:Analisis:Edges per node} se razonó el valor tomado para $\lambda$. Para el calculo del parámetro $\alpha$...
%Incluir calculo del parametro alpha
...
%(Guillermo): Truco: puedes usar \mathrm{} para poner texto dentro del modo matemático y que quede algo mejor. He cambiado el primer 'memoria' para que veas cómo queda.
$\begin{array}{lcl}
...
\begin{figure}[hbtp]
\inputgnuplot{gnuplot/memory-usage}
